 ...
 * @return {string}
 */
const SQL_BING_QUERY_SET_BY_IDS= function (id_formatted_str) {return 'select * from '+TABLE_BING_IMAGE+' where '+KEY_BING_IMAGE_ID+' in ('+id_formatted_str+')';};
...
const database = {
    ...
    // 获取指定的图片序列
    getBingImageByIDs : async function(idsRawString){
        try {
            let tmpIDArray = idsRawString.split(',');
            let tmpFormatQueryString = '';
            if (tmpIDArray.length >= 16){
                return false
            }
            for (let i=0; i<tmpIDArray.length; i++){
                let tmpID = tmpIDArray[i];
                if (tmpID.length === 8 && !isNaN(tmpID)){
                    tmpFormatQueryString = tmpFormatQueryString+','+tmpID
                }
            }
            if (tmpFormatQueryString.length > 0){
                tmpFormatQueryString = tmpFormatQueryString.substring(1, tmpFormatQueryString.length);
            }
            let sql = SQL_BING_QUERY_SET_BY_IDS(tmpFormatQueryString);
            let tmpQueryResult = await query(sql, []);
            if (tmpQueryResult.success){
                let tmpReturn = [];
                for (let i=0; i<tmpQueryResult.result.length; i++){
                    tmpReturn.push(formatImageData(tmpQueryResult.result[i]))
                }
                return tmpReturn;
            }else{
                return false;
