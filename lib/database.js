...
function openConnection() {
    ...
    db = mysql.createConnection({
        host : configJson.database_address,
        user : configJson.database_username,
        password : configJson.database_password,
        database : configJson.database_table_name
    });
    db.on('error', (err) => {
        db = null;
...
const database = {
    ...
    // 获取制定的信息，临时的，获取数据库的原生数据返回
    getBingInfoRaw : async function(page, count){
        page = parseInt(page);
        count = parseInt(count);
        //
        let tmpReturnData = [];
