...
const KEY_BING_IMAGE_SOURCE = 'source';
...
const SQL_BING_QUERY_RANDOM = 'select * from '+TABLE_BING_IMAGE+' order by rand() limit ?'
...
function initRawImageData(rawImage) {
...
const database = {
    ...
    // 获取随机的
    getRandomImage : async function(count){
        const randomData = await query(SQL_BING_QUERY_RANDOM, [count])
